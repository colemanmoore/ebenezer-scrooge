...
export default () => {
    ...
    api.get('/account/:user_id', checkSession, async (req, res) => {
    ...
    api.put('/account/:user_id', checkSession, async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({ user_id: req.params.user_id, balance: req.body.balance })
        } catch (error) {
            console.warn(error)
            res.status(500).json(error)
        }
